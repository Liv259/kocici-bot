...
// --- DATA ---
const KLANY = ["Hromový", "Říční", "Větrný", "Stínový", "Hvězdný"] as const;

const KLAN_KLICOVA_SLOVA: Record<string, string> = {
  Hromový: "hromov",
  Říční: "říčn",
  Větrný: "větrn",
  Stínový: "stínov",
...
async function getJidlo(klan: string): Promise<number> {
  const row = await db.select().from(klanyTable).where(eq(klanyTable.nazev, klan)).limit(1);
  return row[0]?.jidlo ?? 0;
}

async function pridejJidlo(klan: string): Promise<number> {
...
async function uberiJidlo(klan: string): Promise<void> {
  await db.update(klanyTable)
...
function getRoleNames(member: GuildMember): string[] {
  if (member.roles instanceof Object && "cache" in member.roles) {
    return [...(member.roles as any).cache.values()].map((r: any) => r.name.toLowerCase());
  }
  return [];
}

// "čedník" zachytí "účedník" i "učedník"
function jeUcednik(member: GuildMember): boolean {
  return getRoleNames(member).some((r) => r.includes("čedník"));
}

function jeLecitel(member: GuildMember): boolean {
  return getRoleNames(member).some((r) => r.includes("léčitel"));
}

function jeVelitel(member: GuildMember): boolean {
  return getRoleNames(member).some((r) => r.includes("velitel"));
}

function jeKote(member: GuildMember): boolean {
  return getRoleNames(member).some((r) => r.includes("kotě"));
}

function jeMatka(member: GuildMember): boolean {
  return getRoleNames(member).some((r) => r.includes("matk"));
}

function jeStarsi(member: GuildMember): boolean {
  return getRoleNames(member).some((r) => r.includes("starší"));
}

const SEZONA_VYCHOZI = "léto";
const SEZONA_INFO: Record<string, { ikona: string; nazev: string; uspech: number; popis: string }> = {
...
async function getSezona(): Promise<string> {
...
async function setSezona(sezona: string) {
...
async function getPrirazeni(ucednikId: string): Promise<string | null> {
...
function bojovaSance(xpA: number, xpB: number): number {
  const total = xpA + xpB;
...
async function jeMrtvy(userId: string): Promise<boolean> {
  const hrac = await getHrac(userId);
  return hrac.mrtvy === 1;
}

async function setMrtvy(userId: string) {
  await getHrac(userId);
...
function rozpoznejKlan(nazevRole: string): string | null {
  const lower = nazevRole.toLowerCase();
  for (const [klan, klicSlovo] of Object.entries(KLAN_KLICOVA_SLOVA)) {
    if (lower.includes(klicSlovo)) return klan;
  }
  return null;
}

function getKlan(member: GuildMember): string | null {
...
function getKlanFromInteraction(interaction: ChatInputCommandInteraction): string | null {
  const guild = interaction.guild;
  if (!guild || !interaction.member) return null;
...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addUserOption((option) =>
      option.setName("hráč").setDescription("Hráč kterého chceš vyzvat (volitelné)").setRequired(false)
    ),
  new SlashCommandBuilder()
    .setName("osetrit")
    .setDescription("Ošetři jiného hráče bylinami (pouze léčitelé)")
    .addUserOption((option) =>
      option.setName("hráč").setDescription("Koho chceš ošetřit").setRequired(true)
    ),
  ...
  new SlashCommandBuilder()
    ...
    .addUserOption((option) =>
      option.setName("učedník").setDescription("Učedník kterého chceš trénovat").setRequired(true)
    ),
  new SlashCommandBuilder()
    .setName("vycvik_lecitel")
    .setDescription("Trénuj učedníka léčitele (pouze léčitelé)")
    .addUserOption((option) =>
      option.setName("učedník").setDescription("Učedník léčitele kterého chceš trénovat").setRequired(true)
    ),
  new SlashCommandBuilder()
    .setName("sezona")
    .setDescription("Nastav roční období (pouze Hvězdný klan)")
    .addStringOption((option) =>
      option.setName("obdobi").setDescription("Roční období").setRequired(true)
        .addChoices(
          { name: "🌸 Jaro", value: "jaro" },
          { name: "☀️ Léto", value: "léto" },
          { name: "🍂 Podzim", value: "podzim" },
          { name: "❄️ Zima", value: "zima" },
        )
    ),
  new SlashCommandBuilder()
    .setName("smrt")
    ...
    .addUserOption((option) =>
      option.setName("hráč").setDescription("Postava").setRequired(true)
    )
    .addBooleanOption((option) =>
      option.setName("vzkrisit").setDescription("Zrušit smrt a vrátit postavu zpět").setRequired(false)
    ),
  new SlashCommandBuilder()
    .setName("priradeni")
    .setDescription("Přiřaď učedníka mentorovi nebo zrušte přiřazení (pouze velitelé)")
    .addUserOption((option) =>
      option.setName("učedník").setDescription("Učedník k přiřazení").setRequired(true)
    )
    .addUserOption((option) =>
      option.setName("mentor").setDescription("Mentor (vynechej pro zrušení přiřazení)").setRequired(false)
    ),
].map((cmd) => cmd.toJSON());
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (interaction.commandName === "lov") {
    ...
    if (jeZraneny && Math.random() < 0.3) {
      const zraneniPostupuje: Record<string, string> = {
  ...
  // /boj
    ...
    const cilHrac = interaction.options.getUser("hráč") as User | null;
  ...
  // /osetrit
    ...
    const bylinaZraneni: Record<string, { bylina: string; popis: string }> = {
      "škrábanec": {
    ...
    const bylinaNemoc: Record<string, { bylina: string; popis: string }> = {
  ...
  // /stav
    ...
    await interaction.reply(
      `📋 **Stav ${getDisplayName(interaction)}:**\n` +
  ...
  // /vycvik
    ...
    // Otázka pro učedníka
    const otazky: { otazka: string; spravna: string; spatne: string[] }[] = [
      {
    ...
    const neutralniTlacitka = moznosti.map((moznost, i) =>
      new ButtonBuilder()
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(neutralniTlacitka);
    ...
    kolektor.on("collect", async (btnInteraction) => {
      ...
      if (spravne) await pridejXP(ucednikUser.id, 3);
  ...
  // /prehled
    ...
    const KLAN_EMOJI: Record<string, string> = {
      "Hromový": "⚡", "Říční": "🌊", "Větrný": "🌬️", "Stínový": "🌑", "Hvězdný": "⭐"
    };

    function zraneniEmoji(z: string): string {
      if (!z || z.toLowerCase().includes("zdrav")) return "✅";
      if (z.toLowerCase().includes("těžk") || z.toLowerCase().includes("kritick") || z.toLowerCase().includes("vážn")) return "🩸";
      return "🩹";
    }
    ...
    for (const klanNazev of klanyKZobrazeni) {
      ...
      if (clenoveKlanu.size === 0) continue;
  ...
  // /sezona
    ...
    await interaction.reply(
      `${info.ikona} **Hvězdný klan vyhlašuje ${info.nazev}!**\n${info.popis}\n🎯 Šance na úlovek: **${Math.round(info.uspech * 100)} %**`
    );
  }
  ...
  // /smrt
    ...
    if (vzkrisit) {
      ...
      await interaction.reply(
        `✨ **${cilHrac.displayName}** byl/a vzkříšen/a Hvězdným klanem a vrací se zpět mezi živé.`
      );
    } else {
      await updateHrac(cilHrac.id, { mrtvy: 1 });
      await interaction.reply(
        `💀 **${cilHrac.displayName}** odchází do Hvězdného klanu. Jejich příběh zde končí.`
      );
    }
  }
    ...
    await interaction.reply(
      `⚔️ **${mentorUser.displayName}** byl/a přiřazen/a jako mentor učedníka **${ucednikUser.displayName}**.\n` +
